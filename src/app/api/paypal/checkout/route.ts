...
export async function POST(request: NextRequest) {
  try {
    ...
    const subtotal = order.items.reduce((sum, it) => sum + Number(it.price) * it.quantity, 0);
    const shipping = Number(order.shipping || 0);
    const total = Number(order.total);
    const computedTotal = Number((subtotal + shipping).toFixed(2));
    if (Math.abs(computedTotal - total) > 0.01) {
      console.warn('Order total mismatch', { orderId, subtotal, shipping, total, computedTotal });
      return NextResponse.json({ error: 'Valores do pedido não conferem' }, { status: 400 });
    }
    ...
    const items = order.items.map(it => ({
      ...
      unit_amount: { currency_code: PAYPAL_CURRENCY, value: Number(it.price).toFixed(2) },
    ...
    const purchase_units = [
      {
        ...
        amount: {
          ...
          breakdown: {
            ...
            shipping: { currency_code: PAYPAL_CURRENCY, value: shipping.toFixed(2) }
          }
        },
        items
      }
    ...
    if (!createRes.ok) {
      ...
      const unsupported = /UNSUPPORTED_PAYEE_CURRENCY|VUNSUPPORTED_PAYEE_CURRENCY|currency not supported/i.test(text);
      if (fallback && unsupported && fallback !== PAYPAL_CURRENCY) {
        try {
          console.info('Attempting fallback currency', { from: PAYPAL_CURRENCY, to: fallback });

          // Fetch a single conversion rate to avoid rounding inconsistencies
          const rateRes = await fetch(`https://api.exchangerate.host/convert?from=${encodeURIComponent(PAYPAL_CURRENCY)}&to=${encodeURIComponent(fallback)}&amount=1`);
          if (!rateRes.ok) throw new Error('Failed to fetch exchange rate');
          const rateJson = await rateRes.json();
          const rate = Number(rateJson.result);
          if (!rate || Number.isNaN(rate) || rate <= 0) throw new Error('Invalid exchange rate');
          ...
          const itemsFallback = order.items.map(it => {
            ...
            return {
              name: `Item ${it.productId}`,
              unit_amount: { currency_code: fallback, value: convertedUnit.toFixed(2) },
              quantity: String(it.quantity),
              sku: it.productId,
            };
          });

          const newSubtotal = itemsFallback.reduce((s, it) => s + Number(it.unit_amount.value) * Number(it.quantity), 0);
          ...
          const purchase_units_fb = [
            {
              ...
              amount: {
                ...
                breakdown: {
                  item_total: { currency_code: fallback, value: newSubtotal.toFixed(2) },
                  shipping: { currency_code: fallback, value: newShipping.toFixed(2) }
                }
              },
              items: itemsFallback
            }
          ...
          if (retryRes.ok) {
            ...
            if (data2?.id) {
              ...
              } catch (e) { console.error('Failed to save paypalOrderId', e) }
            }
            type PayPalLink = { href?: string; rel?: string; method?: string };
            const approve2 = (data2.links || []).find((l: PayPalLink) => l.rel === 'approve');
      ...
      return NextResponse.json({ error: 'Erro ao iniciar pagamento via PayPal' }, { status: 500 });
    }
    ...
    type PayPalLink = { href?: string; rel?: string; method?: string };
    const approve = (data.links || []).find((l: PayPalLink) => l.rel === 'approve');
    if (!approve) {
    ...
    return NextResponse.json({ error: 'Erro interno ao criar pagamento PayPal' }, { status: 500 });
  }
}
