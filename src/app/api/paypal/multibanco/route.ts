...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!createRes.ok) {
      ...
      console.error(
        "PayPal create order failed (multibanco):",
        createRes.status,
        text
      );
      return NextResponse.json(
    ...
    const confirmRes = await fetch(
      `${PAYPAL_BASE}/v2/checkout/orders/${created.id}/confirm-payment-source`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!confirmRes.ok) {
      ...
      console.error(
        "PayPal confirm payment source failed:",
        confirmRes.status,
        text
      );
      return NextResponse.json(
        { error: "Erro ao confirmar Multibanco" },
        { status: 500 }
      );
    }
    ...
    const payerAction = (confirmed.links || []).find(
    ...
    return NextResponse.json({
      orderId: created.id,
      redirectUrl: payerAction?.href,
      confirmed,
    });
  } catch (err) {
    console.error("Multibanco create error:", err);
    return NextResponse.json(
      { error: "Erro interno ao iniciar Multibanco" },
      { status: 500 }
    );
  }
}
