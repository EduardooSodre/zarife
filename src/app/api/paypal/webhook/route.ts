...
async function verifyWebhook(parsedBody: unknown, headers: Record<string, string>) {
  ...
  try {
    ...
    if (eventType === 'PAYMENT.CAPTURE.PENDING') {
      try {
        ...
        const paymentSource =
          ...
          evt?.resource?.purchase_units?.[0]?.payments?.captures?.[0]?.payment_source;
        const multibanco = paymentSource?.multibanco || evt?.resource?.payment_source?.multibanco;

        if (multibanco) {
          const reference = multibanco.payment_reference || multibanco.paymentReference || null;
          const entity = multibanco.payment_entity || multibanco.paymentEntity || null;
          const barcodeUrl = multibanco.barcode_url || multibanco.BARCODE_URL || null;

          const payload = JSON.stringify([
            { provider: 'paypal', type: 'multibanco', entity, reference, barcodeUrl },
          ]);

          console.info('PAYPAL PENDING: candidateOrderId=', orderId, 'paypalOrderId=', evt?.resource?.id);
          if (orderId) {
            ...
            try {
              await prisma.order.update({ where: { id: orderId }, data: { status: 'PROCESSING', paymentMethod: 'multibanco' } });
            } catch (ee) {
            ...
            try {
              ...
              if (found) {
                const note = `Multibanco instruction - entity: ${entity}, reference: ${reference}, barcode_url: ${barcodeUrl}`;
                try {
                  await prisma.order.update({ where: { id: found.id }, data: { notes: note, status: 'PROCESSING', paymentMethod: 'multibanco' } });
                } catch (ee) {
    ...
    if (eventType === 'CHECKOUT.ORDER.APPROVED' || eventType === 'PAYMENT.CAPTURE.COMPLETED') {
      try {
        ...
        if (orderId) {
          console.info('PAYPAL COMPLETED: resolvedOrderId=', orderId, 'paypalOrderId=', evt?.resource?.id);
          ...
          if (order && (order.status === 'PENDING' || order.status === 'PROCESSING')) {
            ...
            try {
              await prisma.order.update({
                where: { id: orderId },
                data: { status: 'PAID', ...(wasMultibanco ? {} : { paymentMethod: 'paypal' }) },
              });
              console.info('Order updated to PAID', { orderId, previousPaymentMethod: order.paymentMethod, setTo: wasMultibanco ? order.paymentMethod : 'paypal' });
            } catch (ee) {
